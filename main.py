...
def parse_args():
    ...
    # Diversity-Aware Configuration Selector (DACS)
    # Optional enhancement that re-ranks Evolver-produced candidate configurations
    # before the original UCB selector picks the next-episode parent. All defaults
    # are off, so omitting these flags reproduces the original behavior exactly.
    parser.add_argument('--enable_dacs', action='store_true', default=False,
                    help='Enable Diversity-Aware Configuration Selector for Evolver-to-Actor configuration selection.')
    parser.add_argument('--dacs_top_n', type=int, default=8,
                    help='Top-N candidates to keep before greedy diversity selection.')
    parser.add_argument('--dacs_select_k', type=int, default=4,
                    help='Number of diversified candidates to retain for final UCB selection.')
    parser.add_argument('--dacs_alpha_relevance', type=float, default=1.0,
                    help='Weight on relevance term in DACS gain.')
    parser.add_argument('--dacs_beta_diversity', type=float, default=0.5,
